...
import { getShop, getShopAccessToken } from "~/lib/d1";
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  const shop = await requireShopFromSession(request, env);
  assertRequestedShopMatchesSession(String(form.get("shop") ?? "") || null, shop);
  ...
  const accessToken = await getShopAccessToken(env.DB, shop, env.WORKER_DATA_KEY);
  if (!accessToken) throw new Response("Shop not installed", { status: 404 });
  ...
  if (!result || result.userErrors.length > 0) {
    throw new Response(JSON.stringify(result?.userErrors ?? "Unknown billing error"), { status: 502 });
  }
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      ...
      <p className="mt-2 text-slate-600">
        ${data.basePriceUsd}/mo base + ${data.perStatePriceUsd} per additional state. Billed via Shopify Subscription
        Billing.
      </p>
      ...
      <Form method="post" className="mt-8 space-y-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <label className="block">
          <span className="text-sm font-medium text-slate-700">Number of states</span>
          <input
            type="number"
            name="states"
            min={1}
      ...
      <p className="mt-6 text-xs text-slate-500">
        Reminder: OnceTax v0 does not auto-file to state portals. We generate filing-prep PDFs you submit yourself.
      </p>
    </main>
  );
}
