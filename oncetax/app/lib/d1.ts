...
export async function upsertShop(
  db: D1Database,
  input: { shop: string; access_token: string; plan?: string | null },
): Promise<void> {
  const id = `shop_${input.shop}`;
  await db
    .prepare(
      `INSERT INTO shops (id, shop, access_token, plan, installed_at)
       VALUES (?1, ?2, ?3, ?4, ?5)
       ON CONFLICT(shop) DO UPDATE SET
         access_token = excluded.access_token,
         plan = COALESCE(excluded.plan, shops.plan)`,
    )
    .bind(id, input.shop, input.access_token, input.plan ?? null, nowSec())
    .run();
}
...
export async function getShop(
  ...
  const row = await db
    .prepare(`SELECT * FROM shops WHERE shop = ?1 LIMIT 1`)
    .bind(shop)
    .first<ShopRow>();
  return row ?? null;
}
...
export async function getStateTotals(
  db: D1Database,
  shop: string,
): Promise<StateTotalsRow[]> {
  const res = await db
    .prepare(
      `SELECT state,
...
export async function insertFiling(
  db: D1Database,
  row: Omit<FilingRow, "generated_at">,
): Promise<void> {
  await db
    .prepare(
      `INSERT INTO filings
...
export async function listFilings(
  ...
  const res = await db
    .prepare(
      `SELECT * FROM filings WHERE shop = ?1 ORDER BY generated_at DESC`,
    )
    .bind(shop)
    .all<FilingRow>();
  return res.results ?? [];
