...
export default function ReceiptVerifierWidget({
  ...
  useEffect(() => {
    ...
    async function run(): Promise<void> {
      ...
      try {
        ...
        if (!resp.ok) {
          throw new Error(`Verifier returned HTTP ${resp.status}`);
        }
  ...
  if (loading) {
    return (
      <div
        className={`${containerClass} border-slate-200`}
        role="status"
        aria-live="polite"
        aria-busy="true"
      >
  ...
  if (error || !data) {
    return (
      <div
        className={`${containerClass} border-rose-200`}
        role="alert"
        aria-live="assertive"
      >
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div
          className={`flex items-center gap-2 text-sm font-semibold ${
            verified ? 'text-emerald-800' : 'text-rose-800'
          }`}
          role="status"
        >
          {verified ? (
      ...
      <dl className="mt-4 grid grid-cols-1 gap-x-6 gap-y-2 text-xs sm:grid-cols-[max-content_1fr]">
        <dt className="font-medium text-slate-600">Signing key ID</dt>
        <dd className="break-all font-mono text-slate-900">
          {data.signing_key_id}
        </dd>
        <dt className="font-medium text-slate-600">Public key fingerprint</dt>
        <dd className="break-all font-mono text-slate-900">
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 overflow-x-auto rounded-md border border-slate-200">
          <table className="min-w-full divide-y divide-slate-200 text-xs">
            ...
            <thead className="bg-slate-50 text-left font-medium uppercase tracking-wide text-slate-600">
              <tr>
              ...
              {payloadEntries.length === 0 ? (
                ...
                payloadEntries.map(([key, value]) => (
                  <tr key={key}>
                    ...
                    <td className="break-all px-3 py-2 font-mono text-slate-900">
                      {renderValue(value)}
                    </td>
                  </tr>
                ))
