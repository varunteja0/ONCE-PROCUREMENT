...
describe("Receipts page (empty state)", () => {
  it("renders an EmptyState when there are no receipts", () => {
    ...
    expect(screen.getByRole("heading", { level: 1, name: /receipts/i })).toBeInTheDocument();
    ...
    expect(statuses.some((node) => /no receipts|no.*yet/i.test(node.textContent ?? ""))).toBe(true);
  });
});
