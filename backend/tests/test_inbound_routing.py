...
def test_fallback_when_no_rules():
    out = select_rule([], tenant_id="t1", from_address="a@x.com", subject=None, attachment_kinds=set())
...
def test_priority_ordering_first_match_wins():
    ...
    out = select_rule(rules, tenant_id="t1", from_address="a@x.com", subject=None, attachment_kinds=set())
    assert out.id == "r5"
...
def test_from_domain_match():
    ...
    assert (
        select_rule([r], tenant_id="t1", from_address="quote@amtrust.com", subject=None, attachment_kinds=set()).id
        == "r"
    )
...
def test_from_domain_mismatch_falls_through():
    r = _rule(from_domain="amtrust.com")
    out = select_rule([r], tenant_id="t1", from_address="quote@other.com", subject=None, attachment_kinds=set())
    assert out.id == "__fallback__"
...
def test_subject_regex_match():
    ...
    assert (
        select_rule([r], tenant_id="t1", from_address="x@y.com", subject="Renewal COI 2026", attachment_kinds=set()).id
        == "r"
    )
...
def test_subject_regex_no_match():
    ...
    out = select_rule([r], tenant_id="t1", from_address="x@y.com", subject="quote", attachment_kinds=set())
    assert out.id == "__fallback__"


def test_attachment_kind_match():
    r = _rule(att_kind="pdf")
    out = select_rule([r], tenant_id="t1", from_address="x@y.com", subject=None, attachment_kinds={"pdf"})
    assert out.id == "r"
...
def test_invalid_regex_does_not_crash():
    r = _rule(subject="(*)bad")
    out = select_rule([r], tenant_id="t1", from_address="x@y.com", subject="anything", attachment_kinds=set())
    assert out.id == "__fallback__"
...
def test_priority_zero_runs_before_default():
    ...
    out = select_rule(rules, tenant_id="t1", from_address="x@y.com", subject=None, attachment_kinds=set())
    assert out.id == "r0"
