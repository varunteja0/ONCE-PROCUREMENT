...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite+aiosqlite:///:memory:", validation_alias="DATABASE_URL"
    )
    redis_url: str = Field(
    ...
    receipt_signing_key_id: str = Field(
        default="default", validation_alias="RECEIPT_SIGNING_KEY_ID"
    )
    receipt_signing_private_key_pem: str = Field(
        default="", validation_alias="RECEIPT_SIGNING_PRIVATE_KEY_PEM"
    )

    portal_smoke_test_timeout_sec: int = Field(
        default=60, validation_alias="PORTAL_SMOKE_TEST_TIMEOUT_SEC"
    )
    # Phase 4: smoke-test → Slack drift alerting
    slack_webhook_url: str = Field(
        default="", validation_alias="SLACK_WEBHOOK_URL"
    )
    slack_alert_channel: str = Field(
        default="#portal-drift", validation_alias="SLACK_ALERT_CHANNEL"
    )
    ...
    enable_in_process_processor: bool = Field(
        default=False, validation_alias="ENABLE_IN_PROCESS_PROCESSOR"
    )
    enable_tos_risky_platforms: bool = Field(
        default=False, validation_alias="ENABLE_TOS_RISKY_PLATFORMS"
    )
    ...
    stripe_price_setup_id: str | None = Field(
        default=None, validation_alias="STRIPE_PRICE_SETUP_ID"
    )
    stripe_price_monthly_id: str | None = Field(
        default=None, validation_alias="STRIPE_PRICE_MONTHLY_ID"
    )
    ...
    billing_grace_period_days: int = Field(
        default=7, validation_alias="BILLING_GRACE_PERIOD_DAYS"
    )
    stripe_mock_mode: bool = Field(
        default=True, validation_alias="STRIPE_MOCK_MODE"
    )
    # --- /L3.5 billing ---
    ...
    email_from_address: str = Field(
        default="noreply@getonce.com", validation_alias="EMAIL_FROM_ADDRESS"
    )
    email_verification_code_ttl_minutes: int = Field(
        default=15, validation_alias="EMAIL_VERIFICATION_CODE_TTL_MINUTES"
    )
    email_verification_max_attempts: int = Field(
        default=5, validation_alias="EMAIL_VERIFICATION_MAX_ATTEMPTS"
    )
    email_verification_max_codes_per_hour: int = Field(
        default=3, validation_alias="EMAIL_VERIFICATION_MAX_CODES_PER_HOUR"
    )
    onboarding_skip_email_verify: bool = Field(
        default=False, validation_alias="ONBOARDING_SKIP_EMAIL_VERIFY"
    )
    onboarding_session_ttl_minutes: int = Field(
        default=30, validation_alias="ONBOARDING_SESSION_TTL_MINUTES"
    )
    email_dispatcher: Literal["outbox", "resend"] = Field(
        default="outbox", validation_alias="EMAIL_DISPATCHER"
    )
    email_outbox_dir: str = Field(
        default=".once/outbox", validation_alias="EMAIL_OUTBOX_DIR"
    )
    # --- /L3.6 onboarding ---
    ...
    import_max_rows: int = Field(
        default=100_000, validation_alias="IMPORT_MAX_ROWS"
    )
    import_chunk_size: int = Field(
        default=500, validation_alias="IMPORT_CHUNK_SIZE"
    )
    ...
    pdf_extraction_enabled: bool = Field(
        default=True, validation_alias="PDF_EXTRACTION_ENABLED"
    )
    ...
    extractor_timeout_seconds: int = Field(
        default=30, validation_alias="EXTRACTOR_TIMEOUT_SECONDS"
    )
    # --- /L3.8 pdf extraction ---
    ...
    verifier_internal_token: str = Field(
        default="", validation_alias="VERIFIER_INTERNAL_TOKEN"
    )
    # Free-tier monthly call cap stamped on any key created without an
    # explicit cap. Matches the ROADMAP "Free tier + paid >10K/mo"
    # commitment.
    verifier_free_tier_monthly_cap: int = Field(
        default=10_000, validation_alias="VERIFIER_FREE_TIER_MONTHLY_CAP"
    )
    # --- /L6.1 verifier API monetization ---

    # --- L3.9 inbound email ---
    inbound_email_enabled: bool = Field(
        default=True, validation_alias="INBOUND_EMAIL_ENABLED"
    )
    inbound_storage_path: str = Field(
        default="./.once/inbound/", validation_alias="INBOUND_STORAGE_PATH"
    )
    inbound_max_email_size_mb: int = Field(
        default=30, validation_alias="INBOUND_MAX_EMAIL_SIZE_MB"
    )
    inbound_max_attachment_size_mb: int = Field(
        default=25, validation_alias="INBOUND_MAX_ATTACHMENT_SIZE_MB"
    )
    inbound_email_domain: str = Field(
        default="in.getonce.com", validation_alias="INBOUND_EMAIL_DOMAIN"
    )
    postmark_webhook_secret: str | None = Field(
        default=None, validation_alias="POSTMARK_WEBHOOK_SECRET"
    )
    ...
    imap_poll_interval_seconds: int = Field(
        default=60, validation_alias="IMAP_POLL_INTERVAL_SECONDS"
    )
    # --- /L3.9 inbound email ---
    ...
    av_scanner_backend: Literal[
        "clamd", "stub_allow", "stub_deny", "eicar", "noop"
    ] = Field(default="stub_allow", validation_alias="AV_SCANNER_BACKEND")
    av_clamd_host: str = Field(
        default="localhost", validation_alias="AV_CLAMD_HOST"
    )
    av_clamd_port: int = Field(default=3310, validation_alias="AV_CLAMD_PORT")
    av_clamd_unix_socket: str | None = Field(
        default=None, validation_alias="AV_CLAMD_UNIX_SOCKET"
    )
    av_clamd_timeout_sec: float = Field(
        default=5.0, validation_alias="AV_CLAMD_TIMEOUT_SEC"
    )
    ...
    av_fail_closed_on_scanner_error: bool = Field(
        default=True, validation_alias="AV_FAIL_CLOSED_ON_SCANNER_ERROR"
    )
    # --- /L4 antivirus scanner ---
    ...
    db_pool_timeout_sec: float = Field(
        default=30.0, validation_alias="DB_POOL_TIMEOUT_SEC"
    )
    db_pool_recycle_sec: int = Field(
        default=1800, validation_alias="DB_POOL_RECYCLE_SEC"
    )
    db_pool_pre_ping: bool = Field(
        default=True, validation_alias="DB_POOL_PRE_PING"
    )
    db_statement_timeout_ms: int = Field(
        default=30_000, validation_alias="DB_STATEMENT_TIMEOUT_MS"
    )
    # --- /DB connection pool ---
        ...
        if isinstance(value, str):
            ...
            if raw.startswith("["):
                try:
                    ...
                    raise ValueError(
                        f"BACKEND_CORS_ORIGINS is not valid JSON: {exc}"
                    ) from exc
